...
import com.swust.back.config.exception.CommonJsonException;
...
public class CommonUtil {
     ...
     * @return
     */
    public static JSONObject errorMsg(String  errorMsg) {
        JSONObject resultJson = new JSONObject();
        resultJson.put("returnCode", "90000");
        resultJson.put("returnMsg", errorMsg);
        resultJson.put("returnData", new JSONObject());
        return resultJson;
    }
     ...
     * @param totalCount  查询出记录的总条数
     */
    public static JSONObject successPage(final JSONObject requestJson, List<JSONObject> list, int totalCount, float sumMoney) {
        int pageRow = requestJson.getIntValue("pageRow");
        int totalPage = getPageCounts(pageRow, totalCount);
        JSONObject result = successJson();
        JSONObject returnData = new JSONObject();
        ...
        returnData.put("totalCount", totalCount);
        returnData.put("totalPage", totalPage);
        returnData.put("sumMoney", sumMoney);
        result.put("returnData", returnData);
        return result;
    }
